# DEPRACTED = This usage is no longer recommended and will be removed soon. This is an example of the old version. For the new version, please visit https://chainless.trymagic.xyz.

from chainless import Agent, TaskFlow, Tool, AgentProtocol
...
class EchoAgent(AgentProtocol):
    ...
    def start(self, ctx: AgentProtocolRunContext):
...
flow.alias("WikipediaAgentOutput", "WikipediaAgent", "output")
...
flow.step("YouTubeSummary", input_map={"input": "{{YouTubeAgent.output}}"})
...
while True:
    ...
    print("\n--- Final ---")
    # output_data = result["output"]["output"]["content"]
