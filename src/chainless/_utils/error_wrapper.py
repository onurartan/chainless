"""
Error wrapper module for Chainless framework.
This module wraps pydantic-ai errors and converts them to Chainless-specific errors.
"""

import functools
import asyncio
from typing import Any, Callable, Union, Optional
from chainless._utils.exception import (
    ChainlessError,
    AgentExecutionError,
...
from chainless._utils.printing import error_message


def map_pydantic_error_to_chainless(error: Exception) -> ChainlessError:
    """
    Maps pydantic-ai and other third-party errors to Chainless-specific errors.

    Args:
        error: The original error from pydantic-ai or other sources

    Returns:
        ChainlessError: A wrapped Chainless-specific error
    """
...
def chainless_error_handler(
    max_retries: int = 0,
    show_error_details: bool = True,
    return_none_on_error: bool = False,
):
    """
    Decorator that wraps functions to handle and convert errors to Chainless-specific errors.
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs) -> Any:
                ...
                for attempt in range(max_retries + 1):
                    try:
                    ...
                    except ChainlessError:
                        # Already a Chainless error, re-raise
                        raise
                    except Exception as e:
                        last_error = e
                        chainless_error = map_pydantic_error_to_chainless(e)
                        ...
                        if attempt == max_retries or not _is_retryable_error(
                            ...
                            if show_error_details:
                                _display_error(chainless_error)

                            if return_none_on_error:
                                return None
                            else:
                                raise chainless_error
...
def _is_retryable_error(error: ChainlessError) -> bool:
    """
    Determines if an error is retryable.

    Args:
        error: The Chainless error to check
...
def _display_error(error: Union[ChainlessError, Exception]) -> None:
    """
    Displays error information to the user using the existing error_message function.

    Args:
        error: The Chainless error to display
    """
    error_type_map = {
    ...
    error_type_name = error_type_map.get(type(error), "Chainless Error")
