...
from chainless._utils.error_wrapper import chainless_error_handler
...
class GeminiStrategy(ModelCreationStrategy):
    ...
    def create_model(self, model_name: str, **kwargs) -> Tuple[Optional[Any], Optional[dict]]:
        ...
        if not api_key:
            return None, {"status_code": 401, "detail": "No API key provided. Please set GOOGLE_GLA_API_KEY or GOOGLE_API_KEY in your configuration."}
...
@chainless_error_handler(max_retries=1, show_error_details=True)
def get_agent_model(llm_model: str):
    """Create a model instance based on the registry entry."""
    from chainless._utils.exception import NoAPIKeyException, ModelConnectionError
